...
## Web-specific layout (desktop)

Ces règles s'ajoutent à ce qui précède pour les écrans web/desktop ; **les couleurs, la police, les rayons d'arrondi et l'échelle d'espacement restent strictement ceux définis plus haut** — ne pas utiliser une échelle différente entre le web et l'app mobile, même si un outil de génération (Stitch) en propose une autre à chaque nouvelle demande.

- **Grille :** 12 colonnes fluides, gouttières 24px, largeur de contenu maximale 1280px.
- **Marge desktop :** 32px de chaque côté au-delà de 1280px de large.
- **Navigation :** barre horizontale fixe en haut, fond blanc, logo à gauche, liens de navigation centrés, sélecteur de zone + recherche + connexion à droite.
- **Sidebar sticky :** utilisée pour le panier (page boutique) et le récapitulatif de commande (checkout, suivi) — reste visible au scroll sur desktop uniquement, empilée en bas sur mobile.

**Note pour toute future génération Stitch :** toujours fournir ce document (`DjidjiMarket-DESIGN-System.md`) en référence/contexte dans le prompt Stitch plutôt que de laisser l'outil régénérer ses propres tokens Material 3 — sinon chaque nouvel écran repart avec des couleurs et des rayons légèrement différents à corriger manuellement.
