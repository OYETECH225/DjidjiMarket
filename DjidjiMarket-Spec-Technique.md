...
Ajout à l'état "fait" : **Profil client + Mes commandes** (PWA + Flutter), d'après `profil_client`/`historique_de_commandes` (mobile) et `djidjimarket_mon_compte`/`djidjimarket_historique_de_commandes` (web). Mes commandes : onglets En cours/Terminées (`Order::FINAL_STATUSES`/`isFinal()`, exposé en `is_final` dans `OrderResource`), miniature du premier article de la commande (`OrderItemResource::listing_photo_url`). Profil : nom/email modifiables (`PATCH /api/user`, nouveau `ProfileController`) avec contrainte d'unicité sur l'email ; **nouvelle capacité "Devenir vendeur"** — un client peut désormais créer un profil boutique sans nouveau compte : `VendorOnboardingService::createProfile()` promeut son rôle en `vendor`, route/endpoint d'onboarding ouverts à `role:client,vendor` au lieu de `role:vendor` seul. Flutter recharge l'utilisateur en cache après coup (`AuthService::refreshUser()`) pour que la nav/le profil reflètent le nouveau rôle sans reconnexion.

**Bug de sécurité trouvé et corrigé (revue automatique) :** `UpdateProfileRequest`/`Profile::save()` ne validaient pas l'unicité de l'email alors que `users.email` a une contrainte unique en base — une mise à jour vers un email déjà pris remontait une 500 brute au lieu d'une erreur de validation propre. Corrigé avec `Rule::unique('users','email')->ignore(...)` des deux côtés.
...
- Profil : pas d'adresses enregistrées, moyens de paiement, notifications, aide, date de naissance, ni avatar — aucun backend derrière ces éléments des maquettes à ce jour.
- Écrans non encore repris des maquettes `design/ecran-mobile`/`design/ecran-web` : espace vendeur, boutique vendeur (web), inscription vendeur, mon compte (web, formulaire détaillé), paiement optimisé, panier et paiement (web).
